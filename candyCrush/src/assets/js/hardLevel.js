	...
	this.populateLevel = function() {
		...
		for (i = 0; i < this.size; i++) {
			//not use 0          
                ...
                for(i=0;i<this.typesOfGems/2;i++){
                    this.level[Math.round(Math.random()*this.size)] =0 ;
                }
	};
...
//    alert("Resawwwwwwwwap Function Body: "+aID+"  and bID: "+bID);
	...
	this.comboUpdate = function(combo){
        ...
        this.populateFlagArray = function(){
            flagArray=new Array();
            ...
            for(var i=0;i<this.itemByLine;i++){
                for(var j=0;j<this.itemByLine;j++){
                    if(i==0 || i==this.size)
                       flagArray[i][j]=Math.floor(Math.random() * 2);
                    else
                       flagArray[i][j]=1;
                }
            }
